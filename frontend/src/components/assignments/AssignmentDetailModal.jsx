...
function getCreatorName(assignment) {
  // 1. Top-level string provided by the backend serializer
  if (assignment?.created_by_name) return assignment.created_by_name;

  const createdBy = assignment?.created_by;

  // 2. Nested creator object
  ...
  // 3. Numeric ID or missing — never expose raw IDs
  return "Faculty";
}
...
function AssignmentDetailModal({ assignment, onClose }) {
  const [status, setStatus] = useState(
    assignment ? getStatus(assignment.deadline) : null,
  );
  ...
  const attachmentUrl = assignment
    ? assignment.attachment_url || assignment.attachment
    : null;
  const isImage =
    assignment?.attachment_resource_type === "image" ||
    (attachmentUrl && /\.(jpg|jpeg|png|gif|webp)$/i.test(attachmentUrl));

  const deadlineDate = assignment ? new Date(assignment.deadline) : null;
  const deadlineDay = deadlineDate
  ...
  return (
    <AnimatePresence>
      {assignment && status && (
        <motion.div
          key="assignment-detail-backdrop"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-3 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider border ${status.color}`}
                    >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-5 sm:px-8 py-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <InfoCard
                  icon={BookOpen}
                  label="Subject"
                  value={getSubjectDisplay(assignment.subject)}
                  index={0}
                />
                <InfoCard
                  icon={Layers}
                  label="Year"
                  value={`Year ${assignment.target_year ?? "N/A"}`}
                  index={1}
                />
                <InfoCard
                  icon={Award}
                  label="Max Marks"
                  value={assignment.max_marks ?? "N/A"}
                  index={2}
                />
                <InfoCard
                  icon={CalendarDays}
